class ReadingVimrcProgressor {
  ...
  get members() {
    return [...new Set(this.messages.map((mes) => mes.user.login)).values()].filter((m) => m);
  }

  start(id, logURL, vimrcs, part) {
    this.id = id;
    this.logURL = logURL;
    this.vimrcs = vimrcs;
    this.messages = [];
...
module.exports = ReadingVimrcProgressor;
