...
class ReadingVimrcRepos {
  ...
  readNextYAMLData() {
    return fsp.readFile(this.nextYAMLFilePath).then((text) => YAML.safeLoad(text)[0]);
  }

  readArchiveYAMLData() {
    return fsp.readFile(this.archiveYAMLFilePath).then(YAML.safeLoad);
  }

  readTargetMembers() {
    return this.readArchiveYAMLData().then((yaml) => new Set(yaml.map((entry) => entry.author.name)));
  }
  ...
  addWikiEntry(requester, author, urlString, comment) {
    return this.wikiUpdater.updateReposToLatest()
      .then(() => this._addNameToWikiFile(requester, author, urlString, comment))
      .then((needPush) => {
        if (needPush) {
          return this.wikiUpdater.commitAndPush(`Add ${author}`);
        }
      });
  }
  ...
  _addNameToWikiFile(requester, author, urlString, comment) {
    const rawURL = makeRawURL(urlString);
    return fetch(rawURL).then((res) => {
      return res.text();
    }).then((text) => {
      const lineNum = text.split("\n").length;
      return this._updateRequestFile((lines) => {
  ...
  _updateNextYAML(nexts, resultData) {
    ...
    return fsp.readFile(yamlPath).then((currentYaml) => {
      ...
      const hash = isContinuous ? nextVimrcUrls[0].split("/")[6] : null;
